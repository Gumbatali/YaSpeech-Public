...
| `DIARIZATION_QUEUE_URL` | URL YMQ-очереди №1 для диаризации | `yc` (или boto3 SQS API) → `create_queue`, см. cloud-deployment.md |
| `DIARIZATION_QUEUE_ARN` | ARN той же очереди (для YMQ-триггера) | `GetQueueAttributes` (boto3 SQS API) с той же очередью |
| `DIARIZATION_QUEUE_URL_2`, `DIARIZATION_QUEUE_ARN_2` | Очередь №2 (для round-robin-параллелизма) | аналогично, см. cloud-deployment.md |
| `DIARIZATION_QUEUE_URL_3`, `DIARIZATION_QUEUE_ARN_3` | Очередь №3 (для round-robin-параллелизма) | аналогично, см. cloud-deployment.md |
