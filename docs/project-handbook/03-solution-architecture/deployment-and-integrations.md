...
- по кнопке «✨ Улучшить с помощью ИИ» — диаризация, глоссарий, коррекция ASR,
  определение имён спикеров. Сама коррекция реплик идёт через Pro (`yandexgpt`),
  остальные стадии этой кнопки — через Lite (`yandexgpt-lite`);
- по кнопке «Собрать протокол» — итоговый протокол (с map-reduce для длинных встреч),
  модель Lite.

Разделение моделей обосновано бенчмарком `scripts/experiments/llm-refine-bench`:
на коррекции Pro вдвое точнее на числах и суммах, на остальных стадиях выигрыш
не окупает шестикратной разницы в цене.
