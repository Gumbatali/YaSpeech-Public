...
**Качество ASR на реальной речи (WER 31.0% на ES2002b) — доменные рычаги
дают ~4 п.п., дальше упирается в перекрывающуюся речь.** `vad_filter=True` +
`condition_on_previous_text=False` + модель `large-v3` вместо `medium`
(все три — в `run/transcribe.py`) снижают WER до 27.3%, но пропуски слов
(Deletions) как были, так и остаются ~75% всех ошибок — рычаги декодирования
их почти не двигают. Проверили гипотезу напрямую: вырезали из той же записи
два 3-минутных окна с одинаковой плотностью речи, но разной долей
перекрывающихся реплик (44.7% и 4.0% времени — `corpus/extract_window.py`,
измерено по таймкодам эталонных сегментов). WER в окне с перекрытием —
**38.0%**, без него — **28.6%**; доля пропусков — 30.2% и 24.1% слов
соответственно. Разница подтверждает: `Mix-Headset` в AMI сводит все
гарнитуры в одну дорожку, и Whisper (однопоточная модель) не может
распознать двух говорящих одновременно — при наложении обычно берёт
доминирующий голос и полностью теряет второй. Это структурный потолок для
такой формы записи, не баг конфигурации; параметрами декодирования дальше
не лечится.

**Прямое подтверждение: та же встреча, отдельные каналы вместо сведённой
дорожки — WER 27.3% → 20.7%.** AMI публикует и `Mix-Headset` (сумма всех
гарнитур), и отдельные `Headset-N.wav` на каждого участника — соответствие
номера канала участнику нашли эмпирически, по энергии сигнала на известных
по разметке отрезках речи каждого (`corpus/match_headsets.py`,
подтверждено полным перебором соответствий: A→H0, B→H1, C→H2, D→H3).
Построили 4 одноголосых сессии (`corpus/ami-participant-session.mjs`,
своя дорожка + свой эталонный текст, без чужих реплик) и прогнали ASR на
каждой отдельно той же лучшей конфигурацией. Микро-усреднённый WER по
всем четырём — **20.7%** (1513 ошибок / 7305 слов) против **27.3%** на
Mix-Headset той же встречи; доля пропусков — **12.7%** против 20.3%,
почти вдвое меньше. Разница только в способе записи аудио, всё остальное
(люди, содержание, длительность) идентично — это прямое, количественное
подтверждение находки про перекрывающуюся речь, не косвенная гипотеза.
**Практический вывод:** если продукт может получать запись отдельными
каналами по участнику (как на большинстве видеозвонков), а не одной
сведённой дорожкой, качество ASR должно вырасти существенно без каких-либо
