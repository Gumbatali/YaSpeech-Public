...
**Учебный пример** сервиса «запись встречи → готовый протокол»: *SpeechKit* (распознавание речи) + *YandexGPT* (коррекция ошибок ASR, протокол в одном структурированном вызове) + *Object Storage* (промежуточное хранение аудио).
...
*   **Фишка**: LLM-коррекция ошибок ASR по всему транскрипту, с автоматическим чанкингом для длинных встреч.
...
### 5. SpeechKit STT v3
*   Асинхронное распознавание с разметкой по каналам. `speakerLabeling` намеренно отключён — спикеров определяет LLM по тексту, а не акустика.
*   5.1 Постобработка транскрипта.

### 6. YandexGPT: коррекция и протокол
*   Pydantic-схема протокола (`ProtocolOutput`).
*   Промпт: коррекция ASR + протокол; реплики остаются подписаны номером канала.
...
Ноутбук читает ключи из `.env` через `load_dotenv()`. Если вы открываете его в блокнотном сервисе, где `.env`-файл недоступен (Google Colab и т.п.), задайте те же переменные окружения любым способом, который предоставляет сервис.
...
1. **Не выдавайте акустическое угадывание за факт**
   - Диаризация ASR по каналам/паузам (`speakerLabeling` и подобные) часто ненадёжна — не путайте её с реальным распознаванием личности говорящего и не просите LLM «дорисовывать» имена по одному тексту без голоса.
   - Если реальные имена участников критичны для вашего сценария — это отдельная задача (акустическая диаризация специализированным сервисом), не решаемая одним текстовым промптом.
...
5. **Масштабирование**
   - Для длинных встреч (десятки тысяч символов) один вызов не поместится в контекст модели — разбивайте на этапы: коррекция по чанкам отдельно от генерации протокола (см. раздел 6 ноутбука).
